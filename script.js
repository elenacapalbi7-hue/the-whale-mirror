...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: 'Segoe UI', sans-serif; }
        ...
        .brand-header h1 { color: var(--gold); letter-spacing: 3px; font-size: 1.5rem; margin-top: 10px; }
        .tagline { font-size: 0.6rem; color: #666; }
        ...
        .about-box p { font-size: 0.75rem; color: #999; }
        ...
        .table-wrapper { width: 100%; height: 450px; overflow-x: auto; background: #000; border: 1px solid #222; border-radius: 12px; }
        table { width: 100%; border-collapse: collapse; min-width: 950px; }
        th { background: #1a1a1a; color: var(--gold); font-size: 0.65rem; padding: 12px; text-align: left; position: sticky; top: 0; z-index: 10; }
        th small { color: #666; font-weight: normal; font-size: 0.55rem; display: block; }
        td { padding: 12px; border-bottom: 1px solid #111; font-size: 0.72rem; font-family: monospace; white-space: nowrap; }
        .btn-gold { width: 100%; padding: 15px; background: var(--gold); border: none; border-radius: 10px; font-weight: bold; cursor: pointer; color: black; margin-top: 20px; }
        .modal { display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.95); z-index: 1000; }
        .modal-content { max-width: 400px; margin: 20% auto; padding: 25px; background: #0a0a0a; border: 1px solid var(--gold); border-radius: 15px; text-align: center; }
    </style>
</head>
<body>
<div class="container">
    <div class="brand-header">
        <img src="ballena.png" width="80" alt="EMC Logo">
        <h1>WHALE MIRROR</h1>
        <p class="tagline">SISTEMA DE AUDITORÍA BLOCKCHAIN NIVEL 4</p>
    </div>
    <div class="about-box">
        <h2>¿Qué es Whale Mirror?</h2>
        <p>Terminal de inteligencia que monitorea nodos RPC en tiempo real para detectar movimientos de ballenas.</p>
    </div>
...
<script>
    ...
    function renderTable() {
        ...
        tbody.innerHTML = coins.map(s => {
            ...
            return `<tr>
                <td style="color:#d4af37; font-weight:bold;">${s}/USDT</td>
                <td style="color:${isBuy ? '#00ff88' : '#ff4444'}">${isBuy ? 'COMPRA' : 'VENTA'}</td>
                ...
                <td style="color:#00ff88">+${impact}%</td>
                <td>${whaleTypes[Math.floor(Math.random() * whaleTypes.length)]}</td>
                <td>HIGH</td>
                <td style="color:#00ff88">SAFE ✅</td>
                <td><button onclick="openSubscription()" style="background:#d4af37; border:none; padding:5px; border-radius:4px; font-size:0.6rem; cursor:pointer; font-weight:bold;">REPORT +</button></td>
            </tr>`;
        }).join('');
    }
    function openSubscription() {
        document.getElementById('modal-content').innerHTML = `
            <h2 style="color:#d4af37;">WHALE MIRROR PRO</h2>
